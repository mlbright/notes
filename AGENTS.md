...
A Ruby on Rails notes application with companion Android and iOS apps built
using Turbo Native (Hotwire).

## Architecture

- Single-instance deployment with role-based access (administrator + regular
  users)
- Ruby on Rails backend with SQLite database
- Mobile apps via Turbo Native, sharing the web UI with native navigation
  wrappers
- Attachments stored on local disk via Active Storage
- Deploy to a VPS with systemd, running from the git checkout; Caddy (reverse
  proxy + automatic TLS) on a separate machine

## Data model

- **User**: name, email, role (admin/user), session timeout (default 1h),
  preferences
- **Note**: title (optional), body (markdown), pinned, archived, trashed,
  trashed_at, max_size (default 32K), timestamps
...
- Session-based auth for the web app; token-based auth (with refresh tokens) for
  Turbo Native clients
...
- Version history: a snapshot is saved on each edit; users can view diffs and
  restore previous versions
- Adjustable maximum note size with a 32K character default

### Editor

- Click to open a note, edit button (or double-click on desktop) to enter edit
  mode
...
- Complete RESTful JSON API covering all application features (notes CRUD, tags,
  sharing, attachments, search, export)
...
- OpenAPI (Swagger) specification auto-generated from the codebase and kept up
  to date
...
- All files (Ruby, JavaScript, YAML, Markdown, etc.) must be formatted according
  to the dominant community standard for that language (e.g., Standard for Ruby,
  Prettier for JS, markdownlint for Markdown)
- Enforce formatting and linting in CI — PRs that fail checks should not be
  merged
- Follow Rails conventions (REST resources, fat models/thin controllers,
  concerns for shared behavior)
- Write clear, descriptive commit messages; keep commits focused on a single
  change

### Testing

- Comprehensive test suite covering models, controllers, request specs, and
  system/integration tests
- Aim for high coverage on business logic (sharing, permissions, versioning,
  search)
...
- Keep the Rails app and the Turbo Native mobile apps in their own separate
  directories
...
- Production runs directly from the git checkout; all production state (SQLite
  databases, Active Storage files, secrets) lives inside the repo directory and
  migrates with it
- systemd units (Puma web service + S3 backup timer) are generated from
  templates in `deploy/` by `make install` and are the only artifacts outside
...
- Caddy runs on a separate machine as reverse proxy with automatic TLS, reaching
  Thruster over Tailscale; Thruster and Puma both listen on all interfaces
...
- Take inspiration from Google Keep: card-based layout, minimal chrome, fast
  interactions
